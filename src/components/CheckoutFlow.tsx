...
// Kinds come from the shared definitions so checkout and the product page can
// never disagree about how a method is settled.
const MANUAL_METHODS: PaymentMethod[] = PAYMENT_METHOD_DEFS.filter((m) => m.kind === 'manual').map((m) => m.id);
const MERCANTIL_METHODS: PaymentMethod[] = PAYMENT_METHOD_DEFS.filter((m) => m.kind === 'mercantil').map((m) => m.id);
...
export default function CheckoutFlow({ totalUsd, totalBs, discountCode, onComplete, onBack }: CheckoutFlowProps) {
  ...
  /**
   * The methods this store actually offers, filtered by `methods_enabled` and
   * with any dashboard-supplied icons applied. Previously every built-in method
   * was always shown, whatever the dashboard said.
   */
  ...
  useEffect(() => {
    ...
    async function loadPayPal() {
      try {
        ...
        /*
          Load the PayPal SDK with card funding switched on.

          `enable-funding=card` is what surfaces the "Debit or Credit Card"
          button beneath the PayPal one, letting an international customer pay
          by card without a PayPal account. Without it the SDK shows only the
          PayPal button, which is why card payments from abroad were not
          possible even though the account supports them.

          This is PayPal's standard hosted flow, NOT Advanced Card Processing
          (hosted card fields). That variant needs per-account underwriting and
          is not offered to merchants in every country; this works on any
          account and keeps the card details on PayPal's side, so nothing
          sensitive touches this app.

          `disable-funding=credit` drops PayPal's own pay-later credit product,
          which is US-only and confusing next to Cashea.
        */
  ...
  /**
   * The cart in the shape Cashea's SDK expects.
   *
   * Cashea builds its own basket from this payload, so it has to be the same
   * lines and the same prices the customer just agreed to — otherwise they
   * approve an instalment plan for a different order than the one we record.
   * Every field it validates has a fallback: the legacy site's most common
   * failure was a product reaching Cashea with an empty description or image.
   */
  const casheaItems = React.useMemo(
    () =>
      cartItems.map((item) => {
        const prices = calculatePrices(item.base_price_usd || item.USD || 0, rates);
        ...
        return {
          ...
          name,
          sku: (item as any).sku || String(item.ItemID || item.id || ''),
          description: (item as any).description || name,
          imageUrl: item.Image || (item as any).image || '',
          quantity: item.quantity,
          priceUsd: prices.usd,
        };
      }),
  ...
  if (step === 'payment') {
    ...
    const methodLabel =
      paymentMethods.find((m) => m.id === formData.paymentMethod)?.title ?? formData.paymentMethod;
    const manualInstructions = isManual
      ? manualPaymentInstructions(formData.paymentMethod, config)
      : null;
        ...
        */}
        ...
        {isManual && (
          <div className="space-y-6">
            <div className="bg-white rounded-3xl border border-slate-100 p-6 space-y-4 shadow-sm">
              ...
              {/*
                Account details come from store_config, edited in the dashboard.
                They used to be literal strings here — a Zelle address, a
                Binance id, the shop address — so the dashboard fields for them
                (pay_zelle_email, pay_binance_id) changed nothing a customer
                could see. If a method is offered but not configured, we say so
                rather than showing an empty box or, worse, someone else's
                account.
              */}
              {manualInstructions && (
                <div className="space-y-2">
                  {manualInstructions.unconfigured ? (
                    <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
                      <p className="text-sm font-bold text-amber-800">
                        Todavia no publicamos los datos para pagar con {methodLabel}.
                        Escribenos por WhatsApp y te los enviamos enseguida.
                      </p>
                    </div>
                  ) : (
                    <>
                      <p className="text-sm text-slate-600 font-bold">{manualInstructions.intro}</p>
                      <div className="bg-[#fff6fa] rounded-2xl p-4 border border-[#ffe0ef] space-y-2">
                        {manualInstructions.lines.map((line) => (
                          <div key={line.label} className="flex items-start justify-between gap-3">
                            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider shrink-0">
                              {line.label}
                            </span>
                            <span className="font-black text-slate-700 text-sm text-right break-all">
                              {line.value}
                            </span>
                          </div>
                        ))}
                      ...
                      {manualInstructions.note && (
                        <p className="text-xs text-slate-400 font-bold">{manualInstructions.note}</p>
                      )}
                    </>
                  )}
        ...
        {formData.paymentMethod === 'paypal' && (
          <div className="space-y-4">
            <div className="bg-white rounded-3xl border border-slate-100 p-6 shadow-sm space-y-4">
              <h3 className="font-black text-slate-800 text-xl">PayPal o tarjeta internacional</h3>
              <p className="text-sm text-slate-500 font-semibold">
                Paga con tu cuenta PayPal, o con tarjeta de credito o debito
                internacional sin necesidad de tener cuenta. El pago se procesa
                de forma segura en PayPal; nosotros nunca vemos los datos de tu
                tarjeta.
              </p>
        ...
        {formData.paymentMethod === 'cashea' && (
          <CasheaPayment
            ...
            totalUsd={totalUsd}
            cedula={formData.id}
            // Cashea builds its own basket from these, so it has to match ours
          ...
          {paymentMethods.map((method) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                {/* An uploaded image wins over inline SVG, matching how the
                    category and collection icons work. */}
                {method.iconUrl ? (
                  <img
                    src={method.iconUrl}
                    alt=""
